...
<template>
  <div>
    ...
    <section id="log-in" class="flex justify-center min-h-52 py-36">
      <form @submit.prevent="createCategory" method="post" class="bg-white shadow-md border-black rounded-lg overflow-hidden items-center w-1/4 p-8">
        <label for="name" class="block text-base mb-2">Name</label>
        <input type="text" id="name" v-model="categoryData.name" class="border w-full text-base px-3 py-2 rounded shadow focus:outline-none focus:ring-0 focus:border-blue-300" placeholder="Name">

        <label for="description" class="block text-base mb-2 mt-8">Description</label>
        <textarea id="description" v-model="categoryData.description" class="border w-full text-base px-3 py-2 rounded shadow focus:outline-none focus:ring-0 focus:border-blue-300" placeholder="Description"></textarea>
